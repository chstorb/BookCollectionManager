...
namespace BCM.WebFormsApplication.BLL
{
    ...
    public class ShoppingCartActions : IDisposable
    {
        ...
        private BCM.DAL.ApplicationDbContext _db = new BCM.DAL.ApplicationDbContext();
        ...
        {
            ShoppingCartId = GetCartId();
            // Multiply book price by quantity of that book to get        
            ...
            total = (decimal?)(from cartItems in _db.ShoppingCartItems
                               ...
                               select (int?)cartItems.Quantity * cartItems.Book.ListPrice).Sum();
        ...
        {
            using (var db = new BCM.DAL.ApplicationDbContext())
            {
                try
                {
                    ...
                    foreach (var cartItem in myCart)
                    {
                        ...
                        {
                            if (cartItem.Book.ID == CartItemUpdates[i].BookId)
                            {
                                if (CartItemUpdates[i].PurchaseQuantity < 1 || CartItemUpdates[i].RemoveItem == true)
                                {
                                    RemoveItem(cartId, cartItem.Book.ID);
                                }
                                else
                                {
                                    UpdateItem(cartId, cartItem.Book.ID, CartItemUpdates[i].PurchaseQuantity);
                                }
                            }
                        }
                    }
